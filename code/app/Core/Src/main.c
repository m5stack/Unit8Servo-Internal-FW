...
__IO uint32_t uiAdcValueBuf[20];
__IO uint16_t usAdcValue12;
...
//     adcTotal += uiAdcValueBuf[i];
//   }

//   usAdcValue12 = adcTotal / 20;

//   vout = usAdcValue12 * 3.3 / 4095;

//   i_in_value = (vout - 1.24) / 0.5;

// 	HAL_ADC_Start_DMA(hadc, (uint32_t *)uiAdcValueBuf, 20);
// }
...
void Slave_Complete_Callback(uint8_t *rx_data, uint16_t len) 
{
    ...
    for(int i = 0; i < len - 1; i++) {
      ...
      {
      case 1:
        HAL_GPIO_WritePin(GPIOA, (uint16_t)(1 << (rx_data[0]+i - 0x10)), GPIO_PIN_SET);
        break;
      case 0:
        HAL_GPIO_WritePin(GPIOA, (uint16_t)(1 << (rx_data[0]+i - 0x10)), GPIO_PIN_RESET);
        break;
	...
	{
    input_state = HAL_GPIO_ReadPin(GPIOA, (uint16_t)(1 << (rx_data[0] - 0x20)));
    ...
    i2c_temp[1] = ((steer_pulse[((rx_data[0] - 0x60) / 2)] * 10) >> 8)& 0xff;
	...
	{
    i2c1_set_send_data((uint8_t *)&pwm_pulse[(rx_data[0] - 0x90)], 1);
  }       
	else if (len == 1 && (rx_data[0] >= 0xA0 & rx_data[0] <= 0xA3)) 
	{
    readCurrentValue();
...
int main(void)
{
  ...
  while (1)
  {
    if (flag_pwm_mode) {
      ...
      for(int i=0; i<8; i++)
      {
        if(TIM16->CNT <= pwm_pulse[i] && Switch[i]==0 && pwm_disable[i] == 0)
        {
          GPIOA->BSRR = steering_gear[i].GPIO_Pin;
          Switch[i]=1;
        }
        else if(TIM16->CNT > pwm_pulse[i] && Switch[i]==1 && pwm_disable[i] == 0 && TIM16->CNT <= 1000)
        {
